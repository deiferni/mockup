...
define([
  ...
  describe("Select2", function() {
    ...
    it('handles orderable tag drop events', function() {
        var $el = $(
        '<div>' +
        ' <input class="pat-select2"' +
        '        data-pat-select2="orderable: true; tags: Red,Yellow,Blue"' +
        ...
        first_elem.find('div').mousedown();
        // initialize second element as a drop target
        second_elem.trigger($.Event('dropinit'), {drag: $proxy});

        // start dragging to configure tolerance function for dropping
        first_elem.trigger($.Event('dragstart'));
        var $proxy = $('li.dragging');
        expect($proxy.size()).to.equal(1);

        // drag first element over second elements bottom,left position
        second_elem.trigger($.Event('dropstart'), {drag: $proxy,
                                                   target: second_elem});
